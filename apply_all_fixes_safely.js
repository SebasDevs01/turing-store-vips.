...
setTimeout(() => {
    ...
    * { box-sizing: border-box; }
    ...
    h1, h2, h3, h4, p, span { color: inherit; }
            ...
            // 3. Add Desktop Sidebar action buttons (Dashboard + ThemeToggle) safely
            // We search for the exact block representing the Sidebar Header.
            // In rebuild_curso_veo, we injected this:
            // <!-- Header del sidebar -->
            // <div class="p-6 border-b border-premium-border">
            ...
            //   </div>
            // </div>
            // We want to replace exactly this segment until the `</div>` closing `p-6`

            const sidebarHeaderRegex = /<!-- Header del sidebar -->\s*<div class="p-6 border-b border-premium-border">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/;
      ...
      <div class="p-5 border-b border-premium-border bg-premium-card flex flex-col gap-4">
        ...
        <div class="flex items-center justify-between gap-2 mt-2">
            ...
            `;
            content = content.replace(sidebarHeaderRegex, newSidebarHeader);

            fs.writeFileSync(srcFile, content);
            console.log('Fixed Sidebar Layout and Dark Mode Text visibility complete.');
        }, 1000);
    }, 1000);
}, 1000);
